...
export default function Nav({ demoEmail }: { demoEmail: string }) {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur-sm border-b border-border">
      ...
      {/* Mobile menu */}
      {open && (
        <div className="md:hidden border-t border-border px-4 py-4 flex flex-col gap-1 bg-white">
          {links.map(({ label, href }) => (
            <Link
              key={href}
              href={href}
              onClick={() => setOpen(false)}
              className={`text-sm font-medium py-2.5 border-b border-border last:border-0 transition-colors ${
